...
###############################################################################        
    ...
    def _deleteData(self, column, table_name):
        if self._checkConnection():
            cur = self.conn.cursor()
            query = sql.SQL("DELETE FROM {} WHERE mac=%s").format(sql.Identifier(table_name))
            data_string = (column, )
            cur.execute(query, data_string)
            self.conn.commit()
    '''
    ...
    '''
    def deleteData(self, column, table_name):  
        self._deleteData(column, table_name) 
